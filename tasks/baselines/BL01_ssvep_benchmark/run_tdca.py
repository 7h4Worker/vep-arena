...
def run_subject_window_task(task: dict) -> dict:
    ...
    req = EpochRequest(
        ...
        kind="filterbank", n_fbs=n_fbs, extra_samples=n_delay, cache_window=cache_window,
    )
    ...
    for block in blocks:
        ...
        base_runtime = {
            "method": "TDCA", "window": window, "subject": subject, "block": block,
        ...
        trial_rows.append({
            "method": "TDCA", "window": window, "subject": subject, "block": block,
            "accuracy": acc, "itr": float(itr), "samples": spec.classes, "seconds": fit_predict_seconds,
        })
...
def main() -> None:
    ...
    parser.add_argument("--task-name", default="BL01_ssvep_benchmark_tdca")
    parser.add_argument("--subjects", default="1-35")
    parser.add_argument("--blocks", default="1-6")
    parser.add_argument("--windows", default="0.2:0.1:1.5")
    ...
    if args.prebuild_epochs:
        ...
        for window in windows:
            for subject in subjects:
                req = EpochRequest(
                    preset=preset, subject=subject, window=window,
                    kind="filterbank", n_fbs=args.n_fbs, extra_samples=args.n_delay,
