...
from tasks.bessvep.run_35t_scoring_smoke import _git_text, _sha256  # noqa: E402
...
def _eeg_references(train_x: np.ndarray, train_y: np.ndarray, samples: int) -> list[np.ndarray]:
    references = []
    for target in range(35):
        reference = train_x[train_y == target, 0, :, :samples].mean(axis=0)
        references.append(reference - reference.mean(axis=1, keepdims=True))
    return references


def _spectral_references(
    frequency_pairs: tuple[tuple[float, float], ...], samples: int, sampling_rate: int, harmonics: int
) -> list[np.ndarray]:
    ...
    for pair in frequency_pairs:
        ...
        if not active:
            raise ValueError("JBHI35 frequency codebook unexpectedly contains a void-void target.")
        rows = []
        for harmonic in range(1, harmonics + 1):
            for frequency in active:
                rows.append(np.sin(2.0 * np.pi * harmonic * frequency * times))
                rows.append(np.cos(2.0 * np.pi * harmonic * frequency * times))
...
def _run_subject(task: dict[str, object]) -> tuple[str, list[dict[str, object]], list[dict[str, object]]]:
    ...
    methods = tuple(str(value) for value in task["methods"])
    bands = int(task["bands"])
    delay = int(task["delay"])
    components = int(task["components"])
    harmonics = int(task["harmonics"])
    frequency_pairs = tuple(tuple(float(item) for item in pair) for pair in task["frequency_pairs"])
    ...
    for window in windows:
        ...
        for test_block in range(6):
            ...
            train_x = epochs[:, train_blocks].reshape(35 * 5, bands, len(channels), needed)
            train_y = np.repeat(labels, 5)
            test_x = epochs[:, test_block]
            references = {
                "EEG_TDCA": _eeg_references(train_x, train_y, samples),
                "SPECTRAL_TDCA": _spectral_references(frequency_pairs, samples, sampling_rate, harmonics),
            }
            for method in methods:
                model = TDCA(n_components=components, n_delay=delay, fb_weights=filterbank_weights(bands))
                model.fit(train_x, train_y, references[method])
        ...
        units.append(
            {
                ...
                "prediction_count": len(window_rows),
                "expected_predictions": expected,
                "runtime_seconds": time.perf_counter() - started,
                "fold_leakage_check": "pass",
                "status": "complete" if len(window_rows) == expected else "partial",
            }
        )
        rows.extend(window_rows)
        print(f"{subject_id} {window:g}s complete", flush=True)
...
def _plot(summary: pd.DataFrame, output: Path) -> None:
    ...
    axis.set(xlabel="Window (s)", ylabel="Accuracy", ylim=(0, 1), title="JBHI35 five-subject TDCA")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Full TDCA evaluation on the reviewed five-subject JBHI35 package.")
    ...
    methods = tuple(str(value) for value in config["tdca_reference_variants"])
    ...
    if windows != tuple(np.arange(0.2, 2.01, 0.2).round(1)):
        raise ValueError("The full TDCA grid must cover 0.2-2.0 s in 0.2 s increments.")
    if methods != ("EEG_TDCA", "SPECTRAL_TDCA") or len(frequency_pairs) != 35:
        raise ValueError("TDCA method or codebook configuration differs from the reviewed protocol.")
    if configured_frequency_pairs != frequency_pairs:
        raise ValueError("TDCA config codebook differs from the shared final-five historical adapter.")
    if int(config["tdca_n_delay"]) != 2 or int(config["tdca_n_components"]) != 4:
        raise ValueError("TDCA parameters must remain fixed at delay=2 and components=4.")
    ...
    manifest = {
        "status": "running",
        "evidence_role": "Arena TDCA extension on the final historical five-subject set; not a MATLAB or paper TDCA reproduction",
        ...
        "reference_variants": {
            "EEG_TDCA": "class EEG temporal subspace estimated inside each training fold from subband 1",
            "SPECTRAL_TDCA": "zero-phase sine/cosine references from unique active left/right frequencies with three harmonics",
        },
        ...
        "n_delay": int(config["tdca_n_delay"]),
        "n_components": int(config["tdca_n_components"]),
        "parameter_status": "fixed from the existing whole-JBHI TDCA task; not tuned on this five-subject full run",
        "model_label_base": 0,
        ...
        "codebook_source": "package validation/run_2s_etraca_validation.m lists_freqs_L/lists_freqs_R",
        ...
        "expected_units": len(subjects) * len(windows),
    ...
    _plot(summary, figures_dir / "tdca_accuracy.png")
    ...
    if manifest["status"] != "complete":
        raise RuntimeError("Five-subject TDCA full run is partial; inspect manifest.json and errors.log.")
