...
def _run_unit(task: dict) -> list[dict]:
    ...
    for test_idx, block in enumerate(blocks):
        ...
        for method in task["methods"]:
            if method in {"MOHP", "EMOHP"}:
                model = MultiObjectiveHighPassFilter(
                    n_fbs=int(task["n_fbs"]), solver=str(task["solver"]),
                    max_iter=int(task["max_iter"]), ensemble=method == "EMOHP",
                )
            elif method == "TRCA":
            ...
            rows.append(
                {
                    "method": method, "solver": task["solver"], "window": window,
...
def _write(rows: list[dict], result_dir: Path, manifest: dict) -> None:
    ...
    subject = frame.groupby(["method", "solver", "window", "subject"], as_index=False).agg(
        accuracy=("accuracy", "mean"), itr=("itr", "mean"), seconds=("seconds", "sum")
    )
    summary = subject.groupby(["method", "solver", "window"], as_index=False).agg(
...
def main() -> None:
    ...
    parser.add_argument("--windows", default="0.2:0.2:0.8")
    parser.add_argument("--cache-window", type=float)
    parser.add_argument("--methods", default="MOHP,TRCA")
    parser.add_argument("--n-fbs", type=int, default=1)
    parser.add_argument("--solver", choices=["goal_attainment", "weighted_sum"], default="goal_attainment")
    parser.add_argument("--max-iter", type=int, default=100)
    parser.add_argument("--workers", type=int, default=1)
    parser.add_argument("--task-name", default="benchmark_mohp")
    ...
    tasks = [
        {
            ...
            "methods": methods, "n_fbs": args.n_fbs, "solver": args.solver,
    ...
    _write(
        rows, result_dir,
        {
            "task": "M05_mohp", "paper": "10.1109/TIM.2022.3146950",
            ...
            "preprocessing": "BL01 9ch canonical band; cue 0.5 s; latency 0.14 s",
        },
    )
    print(result_dir)
