...
> 范围收束（2026-09-17）：近期主线固定为 SSVEP 数据组织、方法验证和同条件比较；在线回放按实际需求扩展。其他范式和专题是可选扩展，不作为核心交付前置要求。
>
> 当前三层职责以 [architecture.md](architecture.md) 为准；增项验收遵循 [固定回环](integration_workflow_zh.md)，格式遵循 [产物契约](result_artifact_contract.md)。下方保留早期范围/阶段计划和环境记录作为历史背景，其中“当前”“下一个目标”等不应当作今天的完成状态；是否接入完成以带版本和范围的验收证据为准。
